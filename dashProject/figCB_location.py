...
from .diveSiteLocations import diveSiteLoc, diveSitesGroups
...
@app.callback(
    ...
    [Input('xCfgDates', 'data'), Input('xCfgSpecies', 'data')])
def genLocationFigure(xCfgDates, xCfgSpecies):
########################################
########################################
    if xCfgDates == {} or xCfgSpecies == {}:
        log.debug('Not ready to render Location Figure')
        raise dash.exceptions.PreventUpdate()

    log.debug('Rendering Location Figure')
...
#Here we define the updateMenues to be used by the
#location map
global updateMenus
updateMenus = [
    dict(type="dropdown", 
...
button = {
    'method': 'skip',
    'args': [],
    'label' : 'Select Sites', #label the button with the group name
}
...
button = {
    ...
    'args': [{'SelectFromUpdateMenu': True, #signal to callback this is a selection
              'selectedpoints': [list(range(len(dataModule.diveSites)))] #list of indices for this group
             }, 
             [0] #only select curveNumber = 0
            ],
    'label' : 'All Sites', #label the button with the group name
}
updateMenus[0]['buttons'].append(button)

for group, sites in diveSitesGroups.items():

    indices = np.searchsorted(dataModule.diveSites, sites)

    button = {
        'method': 'update',
        'args': [{'SelectFromUpdateMenu': True, #signal to callback this is a selection
                  'selectedpoints': [indices] #list of indices for this group
                 }, 
                 [0] #only select curveNumber = 0
                ],
